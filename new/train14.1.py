# train.py
# Basiert auf der stabilen und effektiven Version 9, erweitert um intelligentes
# Checkpointing und die Unterstützung für das bdh_paper Modell.
...
# Importiere alle Modellarchitekturen
import bdh
import gpt
import bdh_paper # NEU
import requests
...
# --- NEU: Wrapper-Klasse für bdh_paper.BDH_GPU ---
class BDHLanguageModel_Paper(nn.Module):
    def __init__(self, core_config: dict, vocab_size: int):
        super().__init__()
        self.config = core_config
        self.core = bdh_paper.BDH_GPU(n=core_config['n'], d=core_config['d'], V=vocab_size, u_decay=core_config['u_decay'], x_decay=core_config['x_decay'])
        self.head = nn.Linear(core_config['d'], vocab_size, bias=False)
        self.rope = bdh_paper.RotaryEmbedding(core_config['d'])
        # bdh_paper.py hat keine generate-methode, wir fügen eine einfache hinzu
...
def train(args):
    ...
    if args.model_type == 'bdh':
        ...
        model_config = {'n': args.n, 'd': args.d, 'u_decay': 0.97, 'x_decay': 0.97}
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, (x_batch, y_batch) in enumerate(data_loader):
            ...
            optimizer.zero_grad(set_to_none=True) # erst nach allen micro-batches
...
if __name__ == "__main__":
    ...
    # Hyperparameter für bdh und gpt
    parser.add_argument("--d", type=int, default=256)
    parser.add_argument("--n_layer", type=int, default=6)
    parser.add_argument("--n_head", type=int, default=4)
    parser.add_argument("--mlp_multiplier", type=int, default=128)
    parser.add_argument("--dropout", type=float, default=0.1)
    # Hyperparameter NUR für bdh_paper
    parser.add_argument("--n", type=int, default=2048, help="Neuronal dimension for bdh_paper")
