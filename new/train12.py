...
# VERSION 12.1: Setzt die Standard-LR auf 3e-4 zurück, was die Best Practice für den
#               OneCycleLR-Scheduler ist, und behält das flexible Debugging bei.
...
@torch.no_grad()
def debug_model_state(model, step):
    print("\n" + "="*60 + f"\nDEBUGGING MODEL STATE @ STEP {step}\n" + "="*60)
    model_to_inspect = model._orig_mod if hasattr(model, '_orig_mod') else model
    tensors_to_check = {}
    if isinstance(model_to_inspect, bdh.BDH):
        tensors_to_check.update({
            "PARAM_encoder": model_to_inspect.encoder, "PARAM_decoder": model_to_inspect.decoder,
            "PARAM_embed": model_to_inspect.embed.weight, "PARAM_lm_head": model_to_inspect.lm_head,
        })
    elif isinstance(model_to_inspect, gpt.GPT):
         tensors_to_check.update({
            "PARAM_wte": model_to_inspect.transformer.wte.weight, "PARAM_wpe": model_to_inspect.transformer.wpe.weight,
            "PARAM_lm_head": model_to_inspect.lm_head.weight,
        })
    for name, t in tensors_to_check.items():
        if t is None: continue
        if torch.isnan(t).any() or torch.isinf(t).any(): print(f"!!! WARNING: {name} contains NaN/Inf!")
        norm, mean, std = t.norm().item(), t.mean().item(), t.std().item()
        print(f"  {name:<15} | Shape: {str(list(t.shape)):<25} | Norm: {norm:<8.4f} | Mean: {mean:<8.4f} | Std: {std:<8.4f}")
    print("="*60 + "\n")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, (x, y) in enumerate(train_loader):
            ...
            if (step + 1) % gradient_accumulation_steps == 0:
                ...
                if args.debug_layer_interval > 0 and global_step % args.debug_layer_interval == 0:
                    debug_model_state(model, global_step)
...
if __name__ == "__main__":
    ...
    # --- BEST PRACTICE STANDARD-LR für OneCycleLR ---
    parser.add_argument("--lr", type=float, default=3e-4, help="Maximum learning rate for the OneCycleLR scheduler (Best Practice: ~3e-4).")
    ...
    parser.add_argument("--debug_layer_interval", type=int, default=10, help="Log detailed layer stats every N steps. 0 to disable.")
