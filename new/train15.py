# train.py
# VERSION 15: Passt den Wrapper für bdh_paper an die neue, schnelle und
#             stateful Architektur an. Löst den Generate-Bug.

import os, argparse, random, sys, time
from contextlib import nullcontext
import torch, torch.nn as nn
from torch.utils.data import DataLoader
import bdh, gpt, bdh_paper, requests
...
class EpochDataLoader:
    def __init__(self, data, block_size, batch_size):
        ...
        self.num_sequences=len(data)-block_size; self.indices=torch.arange(self.num_sequences)
...
@torch.no_grad()
def evaluate_robust_memorization(model, full_text, stoi, itos, device, epoch, step):
...
# --- ANGEPASST: Wrapper-Klasse für bdh_paper.BDH_GPU ---
class BDHLanguageModel_Paper(nn.Module):
    ...
    def generate(self, idx, max_new_tokens, top_k=1):
        self.eval()
        B, T = idx.shape
        self.core.reset_state(B, idx.device) # WICHTIG: Zustand für die Batch-Größe 1 zurücksetzen

        # "Warm-up" der Zustände mit dem Prompt
        if T > 1:
        ...
        for _ in range(max_new_tokens):
            ...
            # Position ist die aktuelle Länge der Sequenz
...
def train(args):
    # ... (Der Rest der train-Funktion bleibt unverändert) ...
    ...
    elif args.model_type == 'bdh_paper': # NEUE OPTION
        model_config = {'n': args.n, 'd': args.d}
        model = BDHLanguageModel_Paper(model_config, vocab_size).to(DEVICE)
    ...
    for epoch in range(1, args.epochs + 1):
        model.train()
        # WICHTIG für bdh_paper: Reset state at the beginning of each epoch
        if args.model_type == 'bdh_paper':
            model.core.reset_state(micro_batch_size, DEVICE)
        ...
        for step, (x_batch, y_batch) in enumerate(data_loader):
            ...
            if total_loss_in_batch < best_loss:
                ...
                checkpoint = {'model_state_dict': model_to_save.state_dict(), 'config': model_to_save.config, 'stoi': stoi, 'itos': itos, 'model_type': args.model_type}
...
if __name__ == "__main__":
    # ... (Die Argumente bleiben unverändert) ...
    parser = argparse.ArgumentParser(description="Train a language model based on the stable and effective Version 9.")
    parser.add_argument("--run_name", type=str, default="run_" + str(int(time.time())))
    parser.add_argument("--file", type=str, default="input.txt")
    parser.add_argument('--model_type', type=str, default='bdh', choices=['bdh', 'gpt', 'bdh_paper'])
